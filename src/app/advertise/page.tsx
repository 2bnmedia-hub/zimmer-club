...
export default function AdvertisePage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      {/* Hero */}
      <div className="relative bg-espresso text-white py-20 px-4 overflow-hidden">
        <div className="absolute inset-0 opacity-10"
          style={{ backgroundImage: 'url(/hero-bg.png)', backgroundSize: 'cover', backgroundPosition: 'center' }} />
        <div className="relative max-w-3xl mx-auto text-center">
          <Link href="/" className="inline-flex items-center gap-1.5 text-sm text-white/60 hover:text-white mb-8 transition-colors">
            <ArrowRight className="w-4 h-4" />
            חזרה לדף הבית
          </Link>
          <h1 className="text-4xl font-bold mb-4">פרסמו את הנכס שלכם</h1>
          <p className="text-lg text-white/70 max-w-xl mx-auto">
            הצטרפו לרשת הצימרים והנכסים המובילה בישראל — וקבלו חשיפה לאלפי מטיילים בכל חודש
          </p>
          <a href="#form" className="inline-block mt-8 px-8 py-3 rounded-xl font-bold text-white text-sm transition-opacity hover:opacity-90"
      ...
      <div className="max-w-5xl mx-auto px-4 py-16">
        <h2 className="text-2xl font-bold text-gray-900 text-center mb-10">למה לפרסם ב-zimmer.club?</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {BENEFITS.map((b) => (
            <div key={b.title} className="bg-white rounded-2xl p-6 shadow-sm flex gap-4">
              <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0"
                style={{ backgroundColor: '#FDF3DC' }}>
                <b.icon className="w-5 h-5" style={{ color: '#8B6914' }} />
              </div>
              <div>
      ...
      {/* טופס */}
      <div id="form" className="max-w-xl mx-auto px-4 pb-20">
        <div className="bg-white rounded-2xl shadow-sm p-8">
          <h2 className="text-xl font-bold text-gray-900 mb-1">השאירו פרטים</h2>
          <p className="text-sm text-gray-500 mb-6">ניצור איתכם קשר תוך יום עסקים</p>
