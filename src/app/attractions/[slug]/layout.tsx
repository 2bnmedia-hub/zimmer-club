...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${data.name} — אטרקציה ב${data.city || 'ישראל'}`
  const description = data.short_description || data.description?.slice(0, 160) || 'אטרקציה לכל המשפחה בישראל'
  const canonical = `https://www.zimmer.club/attractions/${data.slug || data.id}`
