...
type Attraction = {
  id: string; name: string; city: string; price_per_person: number
  activity_type: string[]; attraction_images?: { url: string }[]; slug?: string
}

const FALLBACK = [
  { id:'1', name:'טיול ג׳יפים בגולן', city:'רמת הגולן', price_per_person:280, tag:'אדרנלין', img:'https://images.unsplash.com/photo-1533130061792-64b345e4a833?w=800&q=80' },
    ...
    <section className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-12 gap-4" style={{ height: '560px' }}>
          {/* כרטיס גדול */}
          {a0 && (
            <div className="col-span-6 group relative rounded-2xl overflow-hidden cursor-pointer" style={{ boxShadow: '0 4px 32px rgba(0,0,0,0.10)' }}>
              ...
              <div className="absolute bottom-0 right-0 left-0 p-6 text-white">
                ...
                <h3 className="font-bold text-xl mb-2">{a0.name}</h3>
            ...
            {[a1, a2].filter(Boolean).map((a, i) => a && (
              <div key={i} className="group relative rounded-2xl overflow-hidden cursor-pointer" style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
                ...
                <div className="absolute bottom-0 right-0 left-0 p-5 text-white">
                  ...
                  <p className="font-bold text-xs" style={{ color: '#F5C842' }}>החל מ ₪{a.price_per_person}</p>
                </div>
              </div>
            ))}
          </div>
