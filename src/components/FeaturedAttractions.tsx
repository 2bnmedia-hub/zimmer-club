...
const FALLBACK = [
  { id:'1', name:'טיול ג׳יפים בגולן', city:'רמת הגולן', price_per_person:280, tag:'אדרנלין', avg_rating: null, img:'https://images.unsplash.com/photo-1533130061792-64b345e4a833?w=1200&q=80' },
  { id:'2', name:'שיט בכנרת', city:'כנרת', price_per_person:180, tag:'רומנטי', avg_rating: null, img:'https://images.unsplash.com/photo-1500514966906-fe245eea9344?w=800&q=80' },
  { id:'3', name:'סדנת בישול איטלקי', city:'תל אביב', price_per_person:320, tag:'תרבות', avg_rating: null, img:'https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=800&q=80' },
]
        ...
        <div className="grid grid-rows-2 gap-3" style={{ height: '520px' }}>
          {a0 && (
            <div className="row-span-1 group relative rounded-2xl overflow-hidden cursor-pointer" style={{ boxShadow: '0 4px 32px rgba(0,0,0,0.10)' }}>
              ...
              {a0.avg_rating && <span className="absolute top-4 left-4 text-xs font-bold px-2.5 py-1 rounded-full" style={{ background: 'rgba(0,0,0,0.45)', color: '#F5C842' }}>★ {Number(a0.avg_rating).toFixed(1)}</span>}
          ...
          <div className="grid grid-cols-2 gap-3">
            {[a1, a2].filter(Boolean).map((a, i) => a && (
              <div key={i} className="group relative rounded-2xl overflow-hidden cursor-pointer" style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
                ...
                {a.avg_rating && <span className="absolute top-3 left-3 text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(0,0,0,0.45)', color: '#F5C842' }}>★ {Number(a.avg_rating).toFixed(1)}</span>}
