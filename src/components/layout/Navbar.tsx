...
export function Navbar() {
  ...
  return (
    <header ref={navRef} className="absolute top-0 left-0 right-0 z-50 bg-white/10 border-none shadow-none" dir="rtl">
...
export function NavbarAuth({ userName, role }: { userName: string; role: 'guest' | 'owner' | 'admin' }) {
  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-50 bg-white/10 border-none shadow-none" dir="rtl">
      <nav className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between h-24">
          <Link href="/" className="shrink-0">
            <img src="/logo.png" alt="Zimmer Club" className="h-16 w-auto logo-shine" />
