          ...
          {p0 && (
            <Link href={`/${p0.slug || p0.id}`} className="col-span-6 row-span-2 group relative rounded-2xl overflow-hidden" style={{ boxShadow: '0 4px 32px rgba(0,0,0,0.10)' }}>
              ...
              <div className="absolute bottom-0 right-0 left-0 p-6 text-white">
                ...
                <h3 className="font-bold text-xl mb-2">{p0.name}</h3>
                <div className="flex items-center justify-between">
                  <p className="text-sm text-white/60">עד {p0.max_guests} אורחים</p>
                  <p className="font-bold" style={{ color: '#F5C842' }}>₪{p0.price_per_night?.toLocaleString()} / לילה</p>
                </div>
              </div>
            </Link>
          )}
          {[p1, p2, p3, p4].filter(Boolean).map((p, i) => p && (
