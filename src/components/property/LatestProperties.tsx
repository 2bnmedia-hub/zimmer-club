...
import { IconStar, IconMapPin, IconPhone, IconChevronLeft, IconMedal, IconHome } from '@/components/icons'
...
const PH_DESTINATIONS = [
  { name: 'גליל עליון', region: 'galil',     img: 'https://images.unsplash.com/photo-1593285942976-70dbd769a590?w=600&q=80', price: '₪850' },
  { name: 'רמת הגולן',  region: 'golan',     img: 'https://images.unsplash.com/photo-1634064866099-30c929edff59?w=600&q=80', price: '₪950' },
  { name: 'ים המלח',   region: 'dead_sea',  img: 'https://images.unsplash.com/photo-1785013045711-c2f5efb1a720?w=600&q=80', price: '₪1,100' },
  { name: 'כרמל',      region: 'carmel',    img: 'https://images.unsplash.com/photo-1652380913410-61c7281c5c97?w=600&q=80', price: '₪780' },
  { name: 'הנגב',      region: 'negev',     img: 'https://images.unsplash.com/photo-1770125853788-6b297d59c912?w=600&q=80', price: '₪720' },
  { name: 'ירושלים',   region: 'jerusalem', img: 'https://images.unsplash.com/photo-1763966461585-41a652f12dd3?w=600&q=80', price: '₪890' },
]
    ...
    <Link href="/advertise" className="col-span-2 row-span-2 group relative rounded-2xl overflow-hidden" style={{ boxShadow: '0 6px 28px rgba(0,0,0,0.14)' }}>
      ...
      <div className="absolute bottom-0 right-0 left-0 p-5 text-white">
        ...
        <span className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full border border-white/30 text-white/90 hover:bg-white/10 transition-colors">
          גלו כיצד <IconChevronLeft size={12} />
        </span>
      </div>
    </Link>
  )
    ...
    <section className="py-10 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href="/search" className="inline-flex items-center gap-1 text-xs font-semibold hover:underline" style={{ color: '#8B6914' }}>כל הנכסים <IconChevronLeft size={12} /></Link>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2.5" style={{ gridAutoRows: '162px' }}>
          ...
          {p0 ? (
            <Link href={p0.slug ? `/properties/${p0.slug}` : `/property/${p0.id}`}
              ...
              <div className="absolute top-3 right-3 flex flex-col gap-1.5">
                ...
                {p0.accepts_miluim && <span className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full bg-white/90" style={{ color: '#1d4ed8' }}><IconMedal size={12} color="#1d4ed8" /> מילואים</span>}
              ...
              {p0.avg_rating && <span className="absolute top-3 left-3 inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(0,0,0,0.5)', color: '#F5C842' }}><IconStar size={11} filled color="#F5C842" /> {p0.avg_rating.toFixed(1)}</span>}
              <div className="absolute bottom-0 right-0 left-0 p-5 text-white">
                <p className="flex items-center gap-1 text-white/55 mb-1 uppercase tracking-widest" style={{ fontSize: '10px' }}><IconMapPin size={11} color="currentColor" /> {p0.city}</p>
                <h3 className="font-bold text-lg leading-tight mb-1">{p0.name}</h3>
                <p className="font-bold text-base" style={{ color: '#F5C842' }}>{p0.price_on_request ? <span className="inline-flex items-center gap-1"><IconPhone size={13} color="#F5C842" /> התקשרו לבירור מחיר</span> : <>₪{p0.price_per_night?.toLocaleString()} <span className="font-normal text-white/50 text-xs">/ לילה</span></>}</p>
              </div>
            </Link>
          ) : <PlaceholderFeatured />}
          ...
          {rest.map((p) => (
            <Link key={p.id} href={p.slug ? `/properties/${p.slug}` : `/property/${p.id}`}
              ...
              {p.avg_rating && <span className="absolute top-2 left-2 inline-flex items-center gap-0.5 font-bold px-1.5 py-0.5 rounded-full" style={{ background: 'rgba(0,0,0,0.5)', color: '#F5C842', fontSize: '10px' }}><IconStar size={9} filled color="#F5C842" /> {p.avg_rating.toFixed(1)}</span>}
              ...
              <div className="absolute bottom-0 right-0 left-0 p-2.5 text-white">
                <p className="flex items-center gap-0.5 text-white/50 leading-none mb-0.5" style={{ fontSize: '9px' }}><IconMapPin size={9} color="currentColor" /> {p.city}</p>
                <h3 className="font-semibold leading-tight line-clamp-1" style={{ fontSize: '12px' }}>{p.name}</h3>
                <p className="flex items-center gap-1 font-bold mt-0.5" style={{ color: '#F5C842', fontSize: '11px' }}>{p.price_on_request ? <><IconPhone size={10} color="#F5C842" /> התקשרו</> : `₪${p.price_per_night?.toLocaleString()}`}</p>
              </div>
            </Link>
          ))}
